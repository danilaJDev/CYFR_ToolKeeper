...
import type { Asset, AssetAuditLog, AssetStatus, AssetTransfer, Location, TransferType } from "@/lib/types";
...
export async function fetchAssets(organizationId: string, params: { search?: string; status?: string; location?: string }) {
  ...
  if (params.status) query = query.eq("status", params.status as AssetStatus);
...
export async function fetchAsset(
  ...
  asset: (Asset & { locations?: { name: string } | null }) | null;
  transfers: (AssetTransfer & { from?: Location | null; to?: Location | null })[];
  audit: AssetAuditLog[];
}> {
  const supabase = await createServerSupabaseClient();
  ...
  const assetRow = (asset ?? null) as (Asset & { locations?: { name: string } | null }) | null;
  const transferRows = (transfers ?? []) as (AssetTransfer & {
    from?: Location | null;
    to?: Location | null;
  })[];
  const auditRows = (audit ?? []) as AssetAuditLog[];

  return { asset: assetRow, transfers: transferRows, audit: auditRows };
}
...
export async function fetchTransfers(
  ...
  if (filters.type) query = query.eq("type", filters.type as TransferType);
